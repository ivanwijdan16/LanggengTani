...
<head>
  ...
  <style>
    ...
    .container {
      width: 100%;
      max-width: 180mm; /* A bit narrower than A4 width to ensure margins */
      margin: 0 auto;
      padding: 15mm 0; /* Top and bottom padding only */
    }
    ...
    @media print {
      @page {
        size: A4;
        margin: 10mm 15mm; /* Standard print margins for A4 */
      }
...
<body>
  <div class="container">
    <div class="header">
      ...
      <p class="report-subtitle">Bulan {{ Carbon\Carbon::parse($pembelians->first()->purchase_date ?? now())->locale('id')->translatedFormat('F Y')}}</p>
    </div>
            ...
            <tr>
              ...
              <td>
                @if($pembelian->stock && $pembelian->stock->masterStock)
                  ...
                  <div style="font-size: 9px; color: #64748b;">{{ $pembelian->stock->masterStock->sku }}</div>
                @else
              ...
              <td class="text-right">Rp {{ number_format($pembelian->purchase_price * $pembelian->quantity, 0, ',', '.') }}</td>
            </tr>
            @php
