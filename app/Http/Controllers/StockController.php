...
class StockController extends Controller
{
  ...
  public function store(Request $request)
  {
    ...
    for ($i = 0; $i < count($validated['name']); $i++) {
      ...
      // Menghasilkan stock_id yang unik berdasarkan nama, ukuran dan tanggal kadaluarsa
      $stock_id = Stock::generateStockId($name, $size, $expiration_date, $sub_type);
      $sku = 'SKU-' . strtolower($name);
      ...
      // Periksa apakah stok dengan stock_id yang sama sudah ada
      $existingStock = Stock::withTrashed()->where('stock_id', $stock_id)->first();
      if ($existingStock) {
  ...
  {
    // Get the last Pembelian ID and increment by 1
    $lastPembelianId = Pembelian::max('id') + 1;

    $pembelianData = [
      'stock_id' => $stock->id,
      'purchase_code' => 'PB-' . now()->format('Ymd') . '-' . str_pad($lastPembelianId, 3, '0', STR_PAD_LEFT),
